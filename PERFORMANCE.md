...
**3-0. 파라미터로 나뉘는 뷰는 "상위 단위 캐시 + 클라 파생"을 검토하라 (todos의 현재 방식)**
캘린더 todos가 실제 사례: 날짜별 쿼리(`['todos', bucketId, date]`)는 캐시 미스마다 왕복이 생겨 날짜 이동이 느렸다. 그런데 서버는 어차피 버킷 전체를 가져와 날짜 필터만 하고 있었음 → **캐시를 버킷 단위(`['todos', bucketId]`)로 올리고 날짜 필터/완료 판정을 클라 파생(`deriveTodosForDate`)으로 이동**. 버킷당 최초 1회만 네트워크, 이후 어떤 날짜든 0-RTT.
- 적용 조건: 서버가 이미 상위 단위로 조회 중이거나, 상위 단위 전체가 전송 가능한 크기일 때.
- 뮤테이션도 단순해짐: 캐시 1곳(`setQueryData`)만 갱신하면 모든 날짜 뷰가 파생으로 일관된다.
  - 토글 = completions 항목 추가/제거(즉시) → 서버 백그라운드 → 실패 시 invalidate
  - 삭제 = todos에서 제거(즉시) → 서버 백그라운드 (3초 대기 → 0ms)
  - 추가 = 서버가 생성 row 반환 → append (재페치 0)
