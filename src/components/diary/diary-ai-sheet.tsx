...
export function DiaryAiSheet({ open, onClose, content, selection, onAddComment }: DiaryAiSheetProps) {
  ...
  async function run(opts: { mode?: DiaryOrganizeMode; question?: string; title: string }) {
    if (loading) return;
    setLoading(true);
    setResult(null);
        ...
        {MODE_ORDER.map((mode) => {
          ...
          return (
            <button
              ...
              disabled={loading}
              className={cn(
                "rounded-lg border border-foreground/15 px-3 py-2.5 text-sm font-medium text-foreground/80 transition-colors",
