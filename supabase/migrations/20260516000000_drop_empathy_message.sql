-- PR 30: stride_plans.empathy_message 완전 제거
--
-- 배경:
--   PR 29에서 대시보드 표시만 숨겼지만 analyzeLifeScene 프롬프트/스키마가 여전히
--   매번 empathyMessage를 생성하여 AI 토큰을 낭비하고 있었음.
--   PR 30에서 발생/저장/표시/타입을 모두 제거하고 DB 컬럼도 함께 정리.
--
-- 변경:
--   1) save_onboarding_journey RPC 재정의 — empathy_message INSERT/UPDATE 제거
--   2) stride_plans.empathy_message 컬럼 DROP
--
-- 데이터 영향: empathy_message는 AI가 자동 생성한 1회분 문구뿐이며 사용자 직접
--   입력이 아니므로 손실 위험 없음. 컬럼은 NOT NULL DEFAULT '' 였음.

-- 1) RPC 재정의: 20260510203000_drop_legacy_profile_columns.sql L15~274 본문에서
--    empathy_message 관련 라인(INSERT 컬럼 / VALUE / DO UPDATE)만 제거.
--    인자 시그니처는 동일하게 유지 — 호출처(saveOnboardingV2Internal) 영향 없음.
...
BEGIN
  ...
  -- stride_plans: empathy_message 컬럼 제거 (PR 30)
...
-- 2) 컬럼 DROP (멱등 — 이미 없으면 무시)
ALTER TABLE public.stride_plans
  DROP COLUMN IF EXISTS empathy_message;
