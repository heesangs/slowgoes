...
daily_todos (
  ...
  stride_level text DEFAULT 'this_month',  -- 현재 CHECK는 this_month 단일값 (20260509 축소)
  sort_order integer DEFAULT 0,
  created_at timestamptz DEFAULT now(),
  completed_at timestamptz
)
...
routines (
  ...
  action_tip text,
  action_tip_generated_at timestamptz,
  time_slot text,                          -- morning | afternoon | evening | night | NULL (20260510)
...
routine_completions (
  ...
  user_id uuid FK → profiles(id),
  week_start date,                         -- 호환용 잔존
  completion_date date NOT NULL,           -- 일별 달성 기록 (20260510140000)
  completed_at timestamptz DEFAULT now(),
  UNIQUE (routine_id, completion_date)     -- 루틴당 하루 1회 완료
)
...
> ⚠️ v1 과제 시스템. 현재 코드에서 참조 없음 — 데이터 보존 목적으로만 테이블 유지.
> 신규 기능은 buckets / daily_todos / routines 체계를 사용한다.
