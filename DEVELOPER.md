...
- 핵심 가치: **"Your values, your pace"** — 사용자 노출 문구는 `APP.TAGLINE` 상수 하나로 관리
...
## 14c. Main Dashboard Design Principles

메인 대시보드를 손볼 때 다음 4가지를 우선한다.

1. **단일 출처 (DRY)** — 같은 정보·기능은 메인에서 한 곳에만 노출한다. 버킷 제목을 인생시계와 현재 버킷 카드에 둘 다 적지 않는다.
2. **얕은 인터랙션 깊이** — 핵심 콘텐츠(발걸음 등)는 메인 표면에. 별도 페이지·중첩 시트는 인라인 또는 단일 시트로 흡수한다.
3. **세분화된 통제 (Granular)** — 발걸음/투두/루틴은 묶음 생성을 지양한다. 부분 재생성 + 새로고침을 기본 인터랙션으로 두고 "한 번에 결정"이 아니라 "조금씩 받고 부담 없이 다시" 흐름을 만든다.
4. **섹션 책임 분리** — 발걸음(시간 지평·미래 비전), 오늘의 한걸음(실행), 숨은 나 찾기(새 영역 탐색)는 책임이 다르다. 한 카드/시트에 섞지 않는다.
